		...
		if (sender instanceof Player)
		{
			...
			else
			{
				...
				if (args.length == 2)
				{
					...
					{
						sender.sendMessage(ChatColor.RED + "La page doit �tre un nombre");
						return true;
					}
				}
				...
				if (topAmount.size() == 0)
				{
					sender.sendMessage(ChatColor.RED + "Il n'y a personne � cette page du classement");
				}
				else
				{
					topList = ChatColor.GREEN + "---- Top " + NKeconomy.currency + ChatColor.GREEN + " ---- " + ChatColor.AQUA + ((page - 1) * 10 + 1) + ChatColor.GREEN + " � " + ChatColor.AQUA + (page * 10) + ChatColor.GREEN + " -----------------------------";
