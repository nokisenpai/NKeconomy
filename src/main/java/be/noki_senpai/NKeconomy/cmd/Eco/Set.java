...
public class Set
{
	...
	public Set(QueueManager queueManager, AccountManager accountManager)
	{
	...
	public boolean set(CommandSender sender, String[] args)
	{
		...
		if(sender instanceof Player)
		{
			...
			if(args.length == 3)
			{
				...
				if(!(sender.hasPermission("*") || sender.hasPermission("nkeco.*") || sender.hasPermission("nkeco.set")
					...
					if(CheckType.isNumber(args[2]))
					{
						queueManager.addToQueue(new Function()
						{
							@Override public Object apply(Object o)
							{
								...
								{
									accountManager.setAmount(args[1], Double.parseDouble(args[2]), false);
									if(Bukkit.getPlayer(args[1]) != null)
									{
										Bukkit.getPlayer(args[1]).sendMessage(
												ChatColor.GREEN + "Vous avez maintenant " + accountManager.format(Double.parseDouble(args[2])) + " "
														+ ConfigManager.CURRENCY);
									}
									sender.sendMessage(ChatColor.AQUA + args[1] + ChatColor.GREEN + " a maintenant "
